...
class King:
    ...
    def one_step_move(self, board, x, y):
        ...
        if x-1 > -1 and board[x-1][y].present == False:
            ...
            if move.valid:
                moves.append(move)
        if y+1 < 8 and board[x][y+1].present == False:
            move = Move(x, y, x, y+1, board)
            if move.valid:
                moves.append(move)
        if y-1 > -1 and board[x][y-1].present == False:
            move = Move(x, y, x, y-1, board)
            if move.valid:
                moves.append(move)
        if x+1 < 8 and y+1 < 8 and board[x+1][y+1].present == False:
            move = Move(x, y, x+1, y+1, board)
            if move.valid:
                moves.append(move)
        if x+1 < 8 and y-1 > -1 and board[x+1][y-1].present == False:
            move = Move(x, y, x+1, y-1, board)
            if move.valid:
                moves.append(move)
        if x-1 > -1 and y+1 < 8 and board[x-1][y+1].present == False:
            move = Move(x, y, x-1, y+1, board)
            if move.valid:
                moves.append(move)
        if x-1 > -1 and y-1 > -1 and board[x-1][y-1].present == False:
            move = Move(x, y, x-1, y-1, board)
            if move.valid:
                moves.append(move)
        return moves
